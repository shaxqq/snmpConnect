...
@Controller('/checkzte')
export class CheckZteController {
    constructor(private checkZteService: CheckZteService) { }
//incoming request and return response
    @Post()
     async baseCheckZte(@Req() req: Request, @Res() res: Response){
      ...
      //console.log('res',this.checkZteService.baseCheckZte(ip, port) )
        let result = await this.checkZteService.baseCheckZte(ip, port) 
